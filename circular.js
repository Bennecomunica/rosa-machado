...
function circularBody(a){var parc=Number(state.config&&state.config.parcelas)||12;var mensalNum=(typeof valorLiquido==='function')?valorLiquido(a):Number(a.mensalidade||0);var tem=Number(a.mensalidade)>0&&mensalNum>0;var mensal=tem?money(mensalNum).replace(/R\$\s*/,''):'_________';var anual=tem?money(mensalNum*parc).replace(/R\$\s*/,''):'_________';var nome=esc(a.nome||'');var nomeForm=nome;var ano=esc(a.serieDestino||'');var periodo=esc(a.periodo||'');var _seg=(a.segmento||'').toString().toLowerCase();var desc30=_seg.indexOf('parcial')>=0?'638,40':(_seg.indexOf('semi')>=0?'841,40':(_seg.indexOf('integral')>=0?'1.017,80':'__________'));var parcelaSeg=_seg.indexOf('parcial')>=0?'912,00':(_seg.indexOf('semi')>=0?'1.202,00':(_seg.indexOf('integral')>=0?'1.454,00':'__________'));return `<div class="page">
  ...
  <div class="form">
    ...
    <div class="opts">
      ...
      <div class="opt"><span class="box">&#9744;</span> Desconto de 30% — R$ ${desc30}, via PIX até <span class="rd">16/10</span>.</div>
      <div class="opt"><span class="box">&#9744;</span> Parcela de R$ ${parcelaSeg}, via Pix até <span class="rd">30/10</span>.</div>
    </div>
